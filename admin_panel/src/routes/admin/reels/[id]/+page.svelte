<script>
  ...
  let editedReel = $state({ ...reel });

  let thumbnailFile = $state(null);
  let videoFile = $state(null);
  let thumbPreview = $state(reel.thumbnail_url);
  let videoPreview = $state(reel.video_url);

  let useExternalThumb = $state(reel.thumbnail_url?.startsWith('http'));
  let useExternalVideo = $state(reel.video_url?.startsWith('http'));
  ...
  function handleFileChange(e, type) {
    ...
    if (type === 'thumb') {
      ...
      videoElement.onloadedmetadata = () => {
        ...
        editedReel.duration = `${minutes}:${seconds.toString().padStart(2, '0')}`;
      };
    }
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      let tUrl = editedReel.thumbnail_url;
...
<div class="edit-reel-page animate-in">
  <div class="page-header">
    ...
    <h1>Edit Wellness Reel</h1>
    <p>Modify the details and media for this clinical short clip.</p>
  </div>
    ...
    <form onsubmit={handleSubmit}>
      <div class="form-grid">
        <div class="input-group full">
          ...
          <input type="text" bind:value={editedReel.title} class="input-field" placeholder="e.g. 5 Mins Healthy Breakfast" required />
        </div>
        <div class="input-group full">
          <label>Category</label>
          <select bind:value={editedReel.category} class="input-field">
            {#each categories as cat}<option value={cat.id}>{cat.name}</option>{/each}
          </select>
        </div>
        <div class="input-group full">
          <label>Description</label>
          <textarea bind:value={editedReel.description} class="input-field" rows="4" placeholder="Provide a brief clinical overview..."></textarea>
        </div>
        ...
        <div class="upload-section full">
          <div class="upload-box">
            ...
            {#if useExternalThumb}
              <input type="text" bind:value={editedReel.thumbnail_url} class="input-field" placeholder="Paste thumbnail URL" />
            {:else}
          ...
          <div class="upload-box">
            ...
            {#if useExternalVideo}
              <input type="text" bind:value={editedReel.video_url} class="input-field" placeholder="Paste video URL" />
            {:else}
...
<style>
  /* Shared styles from create page for consistency */
  ...
  .limit-hint { font-size: 0.75rem; font-weight: 600; color: white; opacity: 0.8; margin-top: -0.25rem; }

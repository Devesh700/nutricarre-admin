<script>
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      delete payload.id; // Remove ID from update payload
      delete payload.created_at;

      const { error } = await supabase
        .from('recipes')
        .update(payload)
        .eq('id', data.recipe.id);
...
<div class="page" style="animation: fadeIn 0.4s ease">
  <div class="page-head">
    <div class="head-left">
      ...
      <h1>Edit Recipe</h1>
      <p>Modifying <strong>{data.recipe.name}</strong></p>
    </div>
  </div>
      ...
      <form onsubmit={handleSubmit} id="recipe-form">
        <div class="form-section">
          ...
          <div class="group">
            ...
            <input type="text" bind:value={recipe.name} required />
          </div>
          <div class="group">
            <label>Short Description</label>
            <textarea bind:value={recipe.description} rows="2"></textarea>
          </div>
        </div>
        ...
        <div class="form-section">
          ...
          <div class="group">
            ...
            <textarea bind:value={recipe.ingredients} rows="8"></textarea>
          </div>
          <div class="group">
            <label>Cooking Instructions</label>
            <textarea bind:value={recipe.instructions} rows="8"></textarea>
          </div>
        </div>
      </form>
    </div>
    ...
    <div class="side-panel">
      <div class="card panel-card">
        ...
        <div class="group">
          ...
          <input type="number" bind:value={recipe.calories} required />
        </div>
      ...
      <div class="card panel-card">
        ...
        <div class="group">
          ...
          <input type="text" bind:value={recipe.image_url} />
        </div>
        {#if recipe.image_url}

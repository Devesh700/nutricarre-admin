...
CREATE TABLE IF NOT EXISTS public.system_settings (
    ...
    is_general_plan_enabled BOOLEAN DEFAULT true,
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.profiles (
    ...
    -- Additional Columns from Live DB
    phone TEXT,
    blood_group TEXT,
    ...
    activity_level TEXT,
    health_goal TEXT,
    has_previous_plan BOOLEAN DEFAULT FALSE,
    therapeutic_appointment_pref TEXT,
    past_diet_year TEXT,
    past_diet_feedback TEXT,
    early_morning_routine TEXT,
    breakfast_routine TEXT,
    mid_morning_routine TEXT,
    lunch_routine TEXT,
    evening_routine TEXT,
    dinner_routine TEXT,
    post_dinner_routine TEXT,
    wake_time TEXT,
    sleep_time TEXT,
    street_address TEXT,
    city TEXT,
...
CREATE TABLE IF NOT EXISTS public.subscription_plans (
    ...
    diet_template_id BIGINT REFERENCES public.diet_templates(id) ON DELETE SET NULL,
...
CREATE OR REPLACE FUNCTION public.get_daily_macros(p_user_id UUID, p_date DATE)
RETURNS JSON AS $$
DECLARE
    v_result JSON;
BEGIN
    ...
    WHERE user_id = p_user_id
      AND logged_at::date = p_date::date;
...
-- RPC: add_video_comment
CREATE OR REPLACE FUNCTION public.add_video_comment(v_id UUID, u_id UUID, c_text TEXT)
RETURNS VOID AS $$
BEGIN
    INSERT INTO public.video_comments (video_id, user_id, comment, content)
    VALUES (v_id, u_id, c_text, c_text);

    UPDATE public.videos SET comments_count = COALESCE(comments_count, 0) + 1 WHERE id = v_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
CREATE OR REPLACE FUNCTION public.toggle_video_like(v_id UUID, u_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
    liked BOOLEAN;
BEGIN
    SELECT EXISTS (
        SELECT 1 FROM public.video_likes WHERE video_id = v_id AND user_id = u_id
    ) INTO liked;

    IF liked THEN
        DELETE FROM public.video_likes WHERE video_id = v_id AND user_id = u_id;
        UPDATE public.videos SET likes_count = GREATEST(COALESCE(likes_count, 0) - 1, 0) WHERE id = v_id;
        RETURN FALSE;
    ELSE
        INSERT INTO public.video_likes (video_id, user_id) VALUES (v_id, u_id);
        UPDATE public.videos SET likes_count = COALESCE(likes_count, 0) + 1 WHERE id = v_id;
        RETURN TRUE;
    END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
CREATE INDEX IF NOT EXISTS idx_step_logs_user ON public.step_logs(user_id);
...
-- Trigger function: generate_schedule_trigger
CREATE OR REPLACE FUNCTION public.generate_schedule_trigger()
RETURNS TRIGGER AS $$
BEGIN
    PERFORM public.generate_user_schedule(NEW.user_id);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    LOOP
        ...
        -- Check achievement criteria based on metric_type
        IF v_achievement.metric_type = 'diet_consistency' THEN
            -- Check if user logged meals on target_value consecutive days
            SELECT COUNT(DISTINCT (logged_at AT TIME ZONE 'Asia/Kolkata')::date)
            INTO v_count
            FROM public.calorie_logs
            WHERE user_id = p_user_id
              AND (logged_at AT TIME ZONE 'Asia/Kolkata')::date >= v_today - (v_achievement.target_value::integer - 1);
            
            IF v_count >= v_achievement.target_value::integer THEN
                v_eligible := TRUE;
            END IF;
        ...
        ELSIF v_achievement.metric_type = 'step_streak' THEN
            -- Check step goal completion for target_value consecutive days
            SELECT COUNT(DISTINCT step_date)
            INTO v_count
            FROM public.step_logs
            WHERE user_id = p_user_id
              AND total_steps >= goal_steps
              AND step_date >= v_today - (v_achievement.target_value::integer - 1);
        ...
        ELSIF v_achievement.metric_type = 'hydration_streak' THEN
            -- Check hydration logging for target_value consecutive days
            SELECT COUNT(DISTINCT (logged_at AT TIME ZONE 'Asia/Kolkata')::date)
            INTO v_count
        ...
        -- If eligible, unlock the achievement!
        IF v_eligible THEN
            -- Record unlock
...
CREATE TRIGGER trg_after_calorie_log_insert AFTER INSERT ON public.calorie_logs FOR EACH ROW EXECUTE FUNCTION public.trg_evaluate_achievements_meal();
